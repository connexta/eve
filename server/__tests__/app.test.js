...
describe("GET ",() => {
    describe("/theme ", () => {
        ...
        test("with existing query", async () => {
            const response = await request(app).get("/theme?wallboard=TV&component=BANNER");
        ...
        test("with non-existing query", async () => {
            const response = await request(app).get("/theme?wallboard=Fake&component=Fake");
            expect(response.body).toEqual({});
            expect(response.statusCode).toBe(200);
        })
    })
...
describe("POST ",() => {
    describe("/theme ", () => {
        ...
        test("with existing query", async () => {
            return request(app).post("/theme?wallboard=TV&component=GITHUB")
            .send(data)
            .then((res)=> {
                expect(res.statusCode).toBe(200);
