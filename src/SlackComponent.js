...
class SlackComponent extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      displayIndex: Array.apply(null, {length: MAX_MSGS}).map(Function.call,Number),
      userLoading: true,
      emojiLoading: true,
      msgLoading: true,
      chanLoading: true
  ...
  async componentDidMount() {
    ...
    this.timerIntervalID = setInterval(() => this.timer(), 
    minute);
  }
  ...
  displayRestOfMessages() {
    let msgToShow = [...this.state.slackMsg.slice(this.state.displayIndex[1]), ...this.state.slackMsg.slice(0, this.state.displayIndex[1])];
    return msgToShow;
  }
